...
function createScene() {
    ...
    ball.update = makeMoveBall(nbrTwists, ballRadius, mobiusHeight);
    updateMobius();
...
function makeMoveOnParametricSurface(f, offset, eps=0.001, klein=false) {
    // f(u,v) defines parametric surface
    // Vector2 dir is direction of move in uv coordinates
    // offset amount from surface
    return function(delta) {
        ...
        let epsx = Math.min(1, pos.x + eps);
        f(pos.x + epsx, pos.y, px);
        px.sub(p);  // gradient in u direction
        let epsy = Math.min(1, pos.y + eps);
        f(pos.x, pos.y + epsy, py);
        py.sub(p);
...
function makeMoveBall(nbrTwists, ballRadius, height) {
    let parametricSurfaceFunction = makeMobiusParametricSurface(radius, nbrTwists, height);
    let klein = (nbrTwists % 2) == 1;
    return makeMoveOnParametricSurface(parametricSurfaceFunction, ballRadius, 0.001, klein);
}
