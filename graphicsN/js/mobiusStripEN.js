...
let ball;
let ballRadius = 0.4;
let mepsilon = 0.0001, bepsilon = 0.35;
...
function initGui() {
    let gui = new dat.GUI();
    gui.add(controls, 'nbrTwists', 0, 13).step(1).onChange(updateMobius);
...
function makeMoveOnParametricSurface(f, offset, eps=0.01, klein=false) {
    ...
    // assumes n-twist klein-cylinder strip
    return function(delta) {
        ...
        let r = 2 * Math.PI * pos.x;
        px = new THREE.Vector3(-Math.sin(r), 0, Math.cos(r));
